...
def create_vocabulary(data, col_header="headers", col_request="params", traffic_source=None):
    corpus = []
    
    IGNORE_LOWER = set(str(k).lower() for k in KEYS_TO_IGNORE)
    GENERALIZE_LOWER = {str(k).lower(): v for k, v in KEYS_TO_GENERALIZE.items()}

    for index, row in tqdm(data.iterrows(), total=data.shape[0], desc="Criando Vocabulário"):
        request_sentence = []
        
        try: 
            raw_headers_dict = safe_json_load(row[col_header])
            if not isinstance(raw_headers_dict, dict):
                continue

            # 1. NORMALIZAÇÃO: Converte todas as chaves para minúsculas
            headers_dict = {str(k).lower(): v for k, v in raw_headers_dict.items()}

            # 2. BUSCA: Agora você busca sempre usando letras minúsculas
            ...
            for key, value in headers_dict.items():
                if key in IGNORE_LOWER:
                    continue

                if key in GENERALIZE_LOWER:
                    request_sentence.append(
                        f"h_{key}={GENERALIZE_LOWER[key]}"
                    )
                else:
                    request_sentence.append(
                        f"h_{key}={value}"
